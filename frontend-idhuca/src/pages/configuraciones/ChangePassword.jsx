...
const ChangePassword = () => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8080/idhuca-indicadores/api/srv/users/get/current', {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          setProfileData(prev => ({
            ...
            currentName: payload.nombre || payload.name || payload.sub || 'Usuario actual'
          }));
        }
      } catch (error) {
  ...
  const changePassword = async (currentPassword, newPassword) => {
    try {
      ...
      const response = await fetch('http://localhost:8080/idhuca-indicadores/api/srv/users/change/password/simple', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (response.ok && data.codigo === 0) {
        ...
        showResponseModal('error', 'Error', data.mensaje || 'Error al cambiar la contraseña');
      }
      
    } catch (error) {
      console.error('Error al cambiar contraseña:', error);
      showResponseModal('error', 'Error de Conexión', 'No se pudo conectar con el servidor. Intente nuevamente.');
    }
  };
  ...
  const updateName = async (userId, newName) => {
    try {
      ...
      const response = await fetch('http://localhost:8080/idhuca-indicadores/api/srv/users/update/name/current', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (response.ok && data.codigo === 0) {
        ...
        showResponseModal('error', 'Error', data.mensaje || 'Error al actualizar el nombre');
      }
  ...
  const handleProfileSubmit = async (e) => {
    ...
    if (!id) {
      displayAlert('danger', 'No se pudo obtener el ID del usuario. Recargue la página e intente nuevamente.');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validatePassword(newPassword)) {
      displayAlert('danger', 'La nueva contraseña no cumple con los requisitos de seguridad.');
      return;
    }
    ...
    if (currentPassword === newPassword) {
      displayAlert('danger', 'La nueva contraseña debe ser diferente a la actual.');
      return;
    }
        ...
        <div className="col-md-8 col-lg-6">
          <div className="card shadow">
            ...
            <div className="card-body p-4">
              ...
              {activeTab === 'profile' && (
                <div>
                  <h6 className="mb-3 text-muted">Actualizar Información Personal</h6>
                  ...
                  <div className="d-flex gap-3 pt-3 border-top">
                    <button 
                      ...
                      disabled={loadingProfile || !profileData.newName.trim() || !profileData.id}
                    >
                      {loadingProfile ? (
                        <>
              ...
              {activeTab === 'password' && (
                <div>
                  <h6 className="mb-3 text-muted">Cambiar Contraseña de Acceso</h6>
      ...
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className={`modal-header ${modalData.type === 'success' ? 'bg-success' : 'bg-danger'} text-white`}>
              <h5 className="modal-title">
                <i className={`fas ${modalData.type === 'success' ? 'fa-check-circle' : 'fa-exclamation-triangle'} me-2`}></i>
                {modalData.title}
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              <div className="text-center py-3">
                <i className={`fas ${modalData.type === 'success' ? 'fa-check-circle' : 'fa-times-circle'} fs-1 ${modalData.type === 'success' ? 'text-success' : 'text-danger'} mb-3`}></i>
            ...
            <div className="modal-footer">
              <button
                type="button"
                className={`btn ${modalData.type === 'success' ? 'btn-success' : 'btn-danger'}`}
                onClick={() => setShowModal(false)}
              >
